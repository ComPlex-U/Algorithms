...
class ArvorePesquisaBinaria:
    ...
    #
    def insert(self, key):
        self.z = self.m.allocate_object()
        self.m.key[self.z] = key
        y = self.NIL
        x = self.root
        
        while x != self.NIL:
            y = x
            if self.m.key[self.z] < self.m.key[x]:
                x = self.m.left[x]
            else:
                x = self.m.right[x]
        ...
        if y == self.NIL:
            self.root = self.z
        elif self.m.key[self.z] < self.m.key[y]:
            self.m.left[y] = self.z
        else:
            self.m.right[y] = self.z
    ...
    def iterativeTreeSearch(self, chave):
